...
if tipo_usuario == 'ALUNO':
    ...
    while True:
        ...
        if opcao_aluno == '1':
            ...
            cursor.execute('''
            SELECT data_pagamento, status_pagamento, valor_pago
            FROM tbl_mensalidade
            INNER JOIN tbl_alunos ON tbl_mensalidade.fk_aluno = tbl_alunos.id_aluno
            WHERE tbl_alunos.nome_aluno = %s
            ''', (usuario,))
...
# =====================================
    ...
    while True:
        ...
        if opcao == '1':
            ...
            data_pagamento = input('Digite a data (AAAA-MM-DD): ')
            ...
            cursor.execute(
                'INSERT INTO tbl_mensalidade (data_pagamento, status_pagamento, valor_pago, fk_aluno) VALUES (%s, %s, %s, %s)',
                (data_pagamento, status, valor_plano, id_aluno)
            )
            conexao.commit()
            ...
            SELECT tbl_alunos.nome_aluno, tbl_mensalidade.status_pagamento,
                   tbl_mensalidade.data_pagamento, tbl_mensalidade.valor_pago
            FROM tbl_mensalidade
            INNER JOIN tbl_alunos ON tbl_mensalidade.fk_aluno = tbl_alunos.id_aluno
            ''')
            ...
            for mensalidade in resultado:
                ...
                print(f'Data: {mensalidade[2]}')
                print(f'Valor: R$ {mensalidade[3]:.2f}')
                print('----------------')
